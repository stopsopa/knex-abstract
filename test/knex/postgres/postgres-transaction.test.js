...
it(`knex - no transaction`, (done) => {
  (async function () {
    ...
    try {
      ...
      expect(String(e)).toContain("does not exist"); // column XXX of relation XXX does not exist

      const count = await man.fetchColumn({}, `select count(*) c from :table: where password in (?)`, [
        ["transa", "transb"],
      ]);
