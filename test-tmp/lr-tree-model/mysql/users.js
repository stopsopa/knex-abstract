...
`,
      ...
      fromDb: async function (row, opt, trx) {
        ...
        if (typeof row.roles === "string") {
          row.roles = row.roles
            .split(",")
            .map((r) => (/^\d+$/.test(r) ? parseInt(r, 10) : r))
            .filter(Boolean);
        }
      ...
      insert: async function (...args) {
        ...
        const id = await prototype.prototype.insert.call(
          this,
          debug,
          trx,
          entity
      ...
      clearRoles: async function (userId) {
        return await this.query(`delete from user_role where user_id = ?`, [
          userId,
        ]);
      },
      find: function (...args) {
...
ORDER BY        id desc
        ...
        return this.raw(debug, trx, query, params)
          .then((data) => {
            // log.dump({
            //     query,
            //     params,
